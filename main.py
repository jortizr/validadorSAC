...
class InterfazApp(ft.UserControl):
    ...
    def search_guia(self, e):
        ...
        print(1)
        #se crea una sesion para reutilizar la conexion
        self.session =requests.Session()
        print(2)
        #se le pasa la guia a la url para completar
        #self.url_guia = 
        #se hace la peticion web a la url
        self.response = self.session.get('http://192.168.1.29/sato/detalleguia.aspx?guia=' + self.txt_guia.value)
        
        # Verificar si la petición fue exitosa (código de estado 200)
        if self.response.status_code == 200:
            self.servidor_status.value = "OK"
            ...
            if self.get_estado:
                ...
                self.guia_estado.value = "GUIA NO EXISTE O MAL DIGITADA"
        else:
            self.servidor_status.value = "Error en la peticion: code " + str(self.response.status_code)
    ...
    def build(self):
        ...
        self.servidor_status = ft.Text("-",  size=16,   color="green", weight=ft.FontWeight.BOLD)
                    ...
                    controls=[
                        ...
                        ft.Row(
                            controls=[
                                ...
                                ft.Text("Servidor: ",  size=16,   color="white", weight=ft.FontWeight.BOLD),
                                self.servidor_status
                            ]
                        ),
                        ft.Divider(height=1)
                    ]
